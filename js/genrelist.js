...
document.addEventListener("DOMContentLoaded", function() {
    ...
    loadGenreList()
    /* 얘네 다 loadGenreList() 함수에 집어넣어서 하드코딩이 되지 않도록 한다.
    axios.get("http://localhost:8585/api/genre/list") // FLO_SERVICE의 GenreAPIController 를 불러옴 (axios. + 매핑종류)
    // axios.get("http://localhost:8585/api/genre/list?page=1") // ?page=1 : 다음 페이지가 나옴
    // axios.get("http://localhost:8585/api/genre/list?keyword=pop") // ?keyword=pop : 키워드가 pop인 장르가 나옴
    .then(function(result) {
        // console.log(result.data.list)
        const genreList = result.data.list
        let tbody_content = "" //view String을 하나 만든다
        for(let i=0; i<genreList.length; i++) {
            // console.log(genreList[i].genreSeq)
            // console.log(genreList[i].genreName)
...
function deleteCategory(no) { // 삭제 버튼 기능 추가(바로 삭제되지 않도록)
    ...
    axios.delete("http://localhost:8585/api/genre/delete?genre_no="+no).then(function(result){
        // console.log(result)
        alert(result.data.message);
        loadGenreList();
    }) // 새로고침 해야 화면에 삭제된 리스트가 뜬다. -> 바로 적용하려고 loadGenreList() 메서드 생성해서 불러옴
}
...
function loadGenreList(page, keyword) { // JavaScript의 파라미터는 꼭 집어넣지않아도 된다.
    //페이지 설정
