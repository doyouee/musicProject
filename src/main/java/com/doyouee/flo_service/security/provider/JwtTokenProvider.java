package com.doyouee.flo_service.security.provider;
...
@Component
public class JwtTokenProvider { // JWT 토큰을 생성해주는 클래스
    private final Key key;
    private final Integer tokenExpireMinutes = 10; // 토큰의 유효시간 10분
    private final Integer refreshExpireMinutes = 60; // 리프레시 토큰의 만료시간 60분
    ...
    public boolean validateToken(String token) { // 토큰 기간 유호한지 검사
        try {
            Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token);
            return true;
        } catch(SecurityException | MalformedJwtException e) {
            System.out.println("Invalid JWT Token"+e);
        } catch(ExpiredJwtException e) { // 만료 된 토큰
            System.out.println("Expired JWT Token"+e);
        } catch(UnsupportedJwtException e) { // 승인되지 않은 토큰
            System.out.println("Unsupported JWT Token"+e);
        } catch(IllegalArgumentException e) { // 등록이 안된 토큰
